...
export const CourseContent = ({ lang }: { lang: Locale }) => {
  ...
  const sortGrid = courses.map((item) => (
    <CourseMidCard key={item.id} item={item} lang={lang} />
  ));

  const sortList = courses.map((item) => (
    <CourseFullCard key={item.id} item={item} lang={lang} />
  ));

  const sortOptions = courseSortOptions.map((item) => (
    <TabsTrigger
      key={item.id}
